...
def get():
  ...
  # Pass Reponse a generator object for it to return a streaming HTTP response.
  return Response(('{0}\r\n'.format(json.dumps(event)) for event in
                   compute.execute_compute_task(stream, namespace,
                                                start_time, end_time,
                                                transforms)),
