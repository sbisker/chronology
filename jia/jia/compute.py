...
class QueryCompute(object):
    ...
    if timeframe['mode'] == 'recent':
      ...
      if bucket_width:
        ...
        if (end_time % bucket_width) != 0:
          ...
          # If bumping the end_time up by one is necessary, we need to add a
          # bucket to the total duration in the next step
          new_buckets = 1

        total_buckets = math.ceil(duration / float(bucket_width)) + new_buckets
        duration = total_buckets * bucket_width
  ...
  def _run_query(self, start_time, end_time):
